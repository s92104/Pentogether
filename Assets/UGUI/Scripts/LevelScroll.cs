...
public class LevelScroll : MonoBehaviour ,IBeginDragHandler,IEndDragHandler {
    ...
    private float[] pageArray = new float[] {0,0.3333f, 0.667f, 1 };
    ...
    public void TurnToPage2(bool isOn)
    {
        if (isOn)
        {
            targetHorizontalPosition = pageArray[1];//跳轉到第2頁
        }
    ...
    public void TurnToPage3(bool isOn)
    {
        if (isOn)
        {
            targetHorizontalPosition = pageArray[2];//跳轉到第3頁
        }
    ...
    public void TurnToPage4(bool isOn)
    {
        if (isOn)
        {
            targetHorizontalPosition = pageArray[3];//跳轉到第4頁
        }
